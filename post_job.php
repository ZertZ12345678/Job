    ...
    if ($job_title && $job_description && $employment_type && $salary && $requirements && $deadline && $payment_method) {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO jobs (company_id, job_title, job_description, location, salary, employment_type, requirements, posted_at, deadline, status)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $company_id, $job_title, $job_description, $company['address'], $salary,
                $employment_type, $requirements, $posted_at, $deadline, $status
            ]);
            ...
            $payment_stmt->execute([
                $company_id, $payment_amount, $payment_status, $job_id, $payment_method
            ]);
...
<head>
    ...
    <style>
        ...
        @media (max-width: 600px) {
            .post-job-container { padding: 20px 7px 20px 7px; }
            .pay-method-img { width: 48px; height: 48px; }
        }
    </style>
</head>
<body>
