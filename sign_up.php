...
require_once "pricing.php"; // defines JH_BASE_FEE and helpers jh_company_discount_for_posts(), jh_price_after_discount()
        ...
        <ul class="navbar-nav align-items-lg-center">
          ...
          <li class="nav-item ms-2">
            <button id="btnInbox" class="btn btn-outline-secondary position-relative" type="button" title="Applications inbox">
              ...
              <?php else: /* render anyway so JS can toggle later */ ?>
  ...
  <script>
    (function() {
      ...
      document.addEventListener('click', function(e) {
        ...
        if (!card) return;
        const id = card.getAttribute('data-id');
        markOne(id); // persist immediately
        paintCardAsRead(card);
        updateBellBadge();
        // allow form submission to keep server session map in sync
      }, false);

      // Mark all (mirror immediately)
      document.addEventListener('click', function(e) {
      ...
      // If server just performed mark-all, mirror after redirect
      if (DID_MARK_ALL) {
        document.querySelectorAll('.app-card').forEach(card => {
          const id = card.getAttribute('data-id');
          markOne(id);
