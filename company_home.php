...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $password     = $_POST['password'] ?? '';   // plain text per request
  ...
  // Uniqueness: email or phone already used?
  if ($message === "") {
  ...
  // Insert (plain text password) + show success message + delayed redirect
  if ($message === "") {
    $stmt = $pdo->prepare("
    ...
    // set session for company_home
    $newId = (int)$pdo->lastInsertId();
    $_SESSION['company_id']   = $newId;
    $_SESSION['company_name'] = $company_name;
    $_SESSION['email']        = $email;

    // show success then redirect via JS after 3s
    $message = "<div class='alert alert-success custom-success text-center'>Registration Successfully and Go to Home Page</div>";
...
<body>
  <div class="signup-container">
    ...
    if (!empty($message)) {
      echo $message;
      if ($register_success) {
        // delay 3s then go to company_home.php
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function check() {
        ...
        toggle(lowerCheck, /[a-z]/.test(v));
        toggle(upperCheck, /[A-Z]/.test(v));
        toggle(numberCheck, /\d/.test(v));
        toggle(specialCheck, /[!@#$%^&*()\-\_\=\+\[\]\{\};:,.?~]/.test(v));
        toggle(spaceCheck, !/\s/.test(v));
      }
