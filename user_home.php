...
try {
  ...
  // fail silently on homepage; optionally log $e->getMessage()
}

// --- 2) Fetch jobs: Active first; Inactive & Closed last; newest first within each group ---
try {
  $sql = "
    SELECT
      j.job_id,
...
<head>
  ...
  <style>
    ...
    .hero-section { background: #f8fafc; padding: 70px 0 50px 0; text-align: center; }
    .search-bar {
      max-width: 700px; margin: 0 auto; margin-top: 30px;
      box-shadow: 0 2px 16px rgba(0,0,0,.06); border-radius: 1.5rem;
      background: #fff; padding: 1.5rem 2rem; display: flex; flex-direction: column; gap: 1rem;
    }
    .search-row { display: flex; flex-wrap: wrap; gap: 1rem; align-items: center; margin-top: .3rem; }
    .search-bar input[type="text"] { flex: 1 1 auto; min-width: 180px; }
    .search-bar select { min-width: 170px; max-width: 230px; }
    .btn-search {
      min-width: 110px; background: #ffc107; color: #fff; border: none; border-radius: .7rem;
      font-weight: 500; font-size: 1rem; padding: .45rem 0; transition: background .18s;
    }
    .btn-search:hover { background: #ff8800; color: #fff; }
    .popular-label { font-size: 1.12rem; color: #22223b; font-weight: 500; margin-right: 12px; }
    .popular-tags { margin-top: 1rem; display: flex; gap: .7rem; justify-content: center; }
    .popular-btn {
      border: 1.7px solid #ffc107; color: #ffc107; background: #fff; font-size: 1.01rem;
      border-radius: .55rem; padding: .33rem 1.25rem; font-weight: 500;
      transition: background .12s, color .12s, border-color .12s;
    }
    .popular-btn:hover { background: #fff8ec; color: #ff8800; border-color: #ff8800; }

    /* Featured job cards */
    .job-card { border: 0; border-radius: 1.25rem; }
    .job-card .logo {
      width: 56px; height: 56px; object-fit: cover; border-radius: .75rem;
      background: #fff; border: 1px solid rgba(0,0,0,.05);
    }
    .job-badge { font-size: .82rem; }

    .footer { background: #1a202c; color: #fff; padding: 30px 0 10px 0; text-align: center; }
  </style>
</head>
...
<body>
  ...
  <section class="hero-section">
    <div class="container">
      ...
      <form class="search-bar" autocomplete="off" method="get" action="search.php">
        <input class="form-control mb-2" type="text" name="q" placeholder="Job title, skills, company...">
  ...
  <section class="py-5">
    <div class="container">
      ...
      <?php if (empty($jobs)): ?>
        <div class="alert alert-light border text-center">No jobs to show yet.</div>
      <?php else: ?>
        <div class="row g-4">
                  ...
                  <div class="d-flex justify-content-between align-items-center">
                    <span class="badge job-badge 
                      <?= $job['status']==='Active' ? 'bg-success' : 
                         ($job['status']==='Inactive' ? 'bg-secondary' : 'bg-danger') ?>">
